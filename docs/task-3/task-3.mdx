...
The requirement for this task was quite broad, so I created a [Datalore notebook](../../assets/duckdb-notebook.zip) that showcases DuckDB’s capabilities in both descriptive and interactive way to serve both as a guide and as a practical tutori
...
I also thought it might be a good idea to show DuckDB's functionality by querying DuckDB’s metadata (DuckDB Aggregate functions overview section). Here you'll a query that dynamically retrieves a list of aggregate functions starting with a specific letter, offering an insightful look into the engine’s functions set (just swap the `function_name LIKE 'a%'` part to any other letter and run the query again).
