...
I thought it might be a good idea to show DuckDB's functionality by querying DuckDB’s metadata (DuckDB Aggregate functions overview section). Here you'll a query that dynamically retrieves a list of aggregate functions starting with a specific letter, which offer a look into the engine’s functions set.
